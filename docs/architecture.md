...
├─ lane/
│  └─ [id]/
│     ├─ index.tsx      "/lane/[id]"          (modal) lane editor; "/lane/new" creates
│     └─ history.tsx    "/lane/[id]/history"  (modal) the lane's finished blocks
...
- **The app tour is NOT a route** — it's a spotlight overlay
  (`src/components/Tour.tsx`: `TourOverlay` mounted in `(app)/_layout.tsx` above the
  Tabs) that dims the live app and cuts a highlight hole around real elements
  (Today's start card / park button / lanes card via `useTourTarget` refs; tab-bar
  slots by computed rect). Started from the "?" button on Today (`useTour.start()`).
  Pure presentation, no data access — keep its step copy in sync when screens change.
...
## 8. Theming & shared UI primitives
...
a bare `ScrollView` — it sets `automaticallyAdjustKeyboardInsets` (iOS scrolls the
focused input above the keyboard; Android resizes via Expo's default
`softwareKeyboardLayoutMode`) and `keyboardShouldPersistTaps="handled"` so buttons
stay tappable while the keyboard is open. This is the app-wide fix for "input hidden
behind the keyboard" — don't solve it per-screen with ad-hoc `KeyboardAvoidingView`s
(nesting them double-shifts the content).
...
- ~~No domain create/archive UI~~ — **done**: full lane CRUD via the `/lane/[id]`
  ...
  gaps.
- **Check-in notification actions aren't handled** (Still on it / Switched / Done
  buttons exist but write nothing). A response listener **does** exist now
  (`useNotificationActions` in `(app)/_layout.tsx`) — it only handles the session
  card's park action so far. See [notifications.md](notifications.md).
- **No live lock-screen timer** — the session lock-screen card is a static
  notification; a real ticking timer needs iOS **Live Activities** (native module →
  custom dev build, not Expo Go). Dev-build follow-up.
